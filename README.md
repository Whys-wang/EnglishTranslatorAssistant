...
> 当前进度:里程碑 9(收尾)—— 完整演示脚本与使用文档。项目主体功能(ASR → 翻译 → 双语字幕 → 纠错 → 可选 TTS)已全部打通。
...
> 用「大模型语音合成 双向流式 V3」(`wss://.../api/v3/tts/bidirection`,事件式二进制协议)
> 合成音频,经 WS 以 `tts_audio`(base64)下发,前端 offscreen 用 Web Audio 顺序播放。
> **默认关闭**;未配置音色时仅打印一次性提示、不影响字幕。每句一条独立连接,延迟优化留到里程碑 8。
...
> **① ASR 修订重译**:当 ASR 对某条已定稿、已翻译的分句又返回了不同原文时,后端判定为「修订」,自动重新翻译并以同 `segment_id` 原地替换;**② 周期性 LLM 复审**:每隔 `ReviewInterval`(默认 3s)取最近 `ReviewContextWindow`(默认 5)段「原文+译文」整体送 LLM,借**后文澄清前文**(歧义、人名、术语)校正早先译文,仅覆盖确有改进且期间未被更新过的句子。两种纠错回填时都带 `corrected` 标记,前端对应字幕会短暂高亮(黄框 + 闪现)提示「这句刚被更新」。窗口内容未变化时跳过复审以节省接口调用。

## 成品预览

下图是实际运行时的界面:左上角是扩展弹窗(开始/停止),页面底部是双语字幕 overlay。
注意第二行被纠错修订的句子有**黄色高亮**(里程碑 6),最后一行是定稿但译文回填中的**「翻译中…」呼吸占位**(里程碑 5)。

![成品预览:弹窗 + 双语字幕 overlay](docs/product-preview.png)

字幕从上到下按时间排列(最新一句最醒目),只保留最近 4 行;原文弱化、中文译文为主,形成清晰的双语层次。
...
### 0. 演示前准备(开场前完成)
1. 在 `backend/internal/config/config.go` 填好真实密钥(见上方「配置」),需要听语音再把 `TTS.Enable=true` 且填好 `TTSVoiceType`。
2. 启动后端:
   ```bash
   cd backend && go run .
   ```
...
4. 打开一个**有外语音频**的标签页(英文演讲 / 播客 / 视频),先**手动播放**确认有声音。

### 1. 开场(15 秒)
> 「这是一个 Chrome 扩展 + Go 后端的实时同声传译助手:它抓取当前标签页的外语音频,
> 实时转写、翻译成中文双语字幕,并能自动纠正先前的识别/翻译错误,还可选朗读译文。」

### 2. 点开始,展示实时字幕(1.5 分钟)
1. 点扩展图标 →「开始翻译」,弹窗状态变为「已开始」。
2. 让音频继续播放,引导观众看页面底部字幕:
   - **中间结果(partial)**:灰显斜体,随说话不断刷新;
   - **定稿(final)**:原文弱化、中文译文加粗为主;
   - **回填**:刚定稿时若译文未到,显示「翻译中…」呼吸占位,译文一到无缝替换。
...
2. **周期性 LLM 复审**:每隔几秒把最近几句「原文+译文」整体送 LLM,借后文校正前文的人名/术语/歧义。
3. 两种纠错回填时,该句字幕会**短暂黄框高亮 + 闪现**,提示「这句刚被更新」——现场指给观众看这个高亮。
...
- **字幕不出现?** 先确认后端在跑(`/healthz` 为 ok)、扩展弹窗显示「已开始」、且标签页**确实在出声**;再看后端日志是否有 `audio received` 帧统计与 `asr handshake`。
