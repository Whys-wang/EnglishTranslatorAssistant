...
以下为**实际运行**录屏(俄→中示例):实时滚动字幕、桌宠控制与纠错高亮。

<video controls src="docs/demo-run.mp4" style="max-width: 100%;"></video>

若上方播放器无法加载,可 [直接打开 demo-run.mp4](docs/demo-run.mp4)。

### 界面截图

桌宠控制台(含 `≥10秒` 静音下限提示) + 扩展已加载 + 页面底部**滚动累积中文字幕**。快捷键以浏览器扩展快捷键页为准(默认 `Alt+Shift+S`)。
