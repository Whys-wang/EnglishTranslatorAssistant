...
> **① ASR 修订重译**:当 ASR 对某条已定稿、已翻译的分句又返回了不同原文时,后端判定为「修订」,自动重新翻译并以同 `segment_id` 原地替换;**② 周期性 LLM 复审**:每隔 `ReviewInterval`(默认 3s)取最近 `ReviewContextWindow`(默认 5)段「原文+译文」整体送 LLM,借**后文澄清前文**(歧义、人名、术语)校正早先译文,仅覆盖确有改进且期间未被更新过的句子。两种纠错回填时都带 `corrected` 标记,前端对应字幕会以**绿色底色**长亮提示「这句刚被更新」。窗口内容未变化时跳过复审以节省接口调用。
...
- **桌宠**:选源/目标语言、开始/停止、字幕颜色、快捷键(默认 `Alt+Shift+S`,可在浏览器扩展快捷键里改)。
- **字幕条**:只显示目标语(中文),左对齐、底部锚定、向上滚动;可拖动位置、可选中复制;停止翻译时才清空。
- **多语种**:英/日/韩/法/德/俄等→中,各语种独立策略(见后端 `segment.go`)。
...
### 2. 点开始,展示实时字幕(1.5 分钟)
1. 点击页面**桌宠**展开面板 → 选源语言/目标语言(如俄语→中文)→「**开始翻译**」,或按快捷键(如 `Ctrl+Z` / `Alt+Shift+S`)。
